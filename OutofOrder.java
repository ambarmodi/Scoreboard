...
public class OutofOrder {
	...
	public static StringBuilder ooorderResult = new StringBuilder();
	
	public static void main(String[] args) {
		String inputFile = args[0];
		String outOfOrderOutputFile = args[2];
		
		BufferedReader bufferedReader = null;
		BufferedWriter ooBufferedWriter = null;
		
		try {
			FileReader fileReader = new FileReader(inputFile);
			bufferedReader = new BufferedReader(fileReader);

			File oofile = new File(outOfOrderOutputFile);
			if (oofile.exists()) {
				oofile.delete();
			}
			FileWriter ooFileWriter = new FileWriter(outOfOrderOutputFile, true);
			ooBufferedWriter = new BufferedWriter(ooFileWriter);
			...
			while ((instr_seq = bufferedReader.readLine()) != null) {
				...
				while(!checkFalseDependency(instruction)) {
					checkCompletion();
					issueInstruction();
					if (DUMP.contains(clock_cycle)) {
						System.out.println("Dumping the scoreboard data:");
						ooorderResult.append("Dumping the scoreboard data:\n");
						dump();
					}
					advanceClock();
				}
		...
		try {
			...
			ooBufferedWriter.write(ooorderResult.toString());
			ooBufferedWriter.close();
	...
	private static void printing() {
		...
		for (Entry<Integer, Unit> entry : scoreBoard.entrySet()) {
			...
			ooorderResult.append(result);
			ooorderResult.append("\n");
		}
		System.out.println("IQ:");
		ooorderResult.append("IQ");
		for (Entry<Integer, Unit> entry : issuedSequence.entrySet()) {
			Unit temp = entry.getValue();
			String result = "   " + temp.getIndex() + " : " + temp.getName()
					+ " : " + temp.getStatus() + " : CT=" + temp.getLatency();
	 ...
	 * @return true if instruction has no dependency else return false.
	 */
	private static boolean checkFalseDependency(Unit instruction) {
		// Return true for No Dependency...else false
		...
		for (Entry<Integer, Unit> entry : issuedSequence.entrySet()) {
			...
			if(instruction.getName().equals("ST") && prev_instr.getName().equals("ST")){
				...
				prev_src3=((PipelinedVariable)prev_instr).getSrc3();
				// check for anti dependencies:W_A_R
				if (!prev_instr.getStatus().equals(STATUS_COMPLETE)
						&& ((prev_src1.equals(dest) || prev_src2.equals(dest)
								|| prev_src3.equals(dest)))) {
					return false;
				}
			} else {
				// check for output, anti dependencies: W-A-W  W-AR 
				if (!prev_instr.getStatus().equals(STATUS_COMPLETE)
						&& ((prev_dest.equals(dest) || prev_src1.equals(dest) || prev_src2
									.equals(dest)))) {
					return false;
				}
			}
	...
	private static Unit addUnit(String instr) {
		// TODO: check if need to put the inst in instrucutin seq
